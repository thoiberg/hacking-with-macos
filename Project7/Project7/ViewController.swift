...
class ViewController: NSViewController, NSCollectionViewDataSource, NSCollectionViewDelegate {
    ...
    func performExternalDrag(with items: [NSPasteboardItem], at indexPath: IndexPath) {
        ...
        for item in items {
            guard let stringURL = item.string(forType: NSPasteboard.PasteboardType(kUTTypeURL as String)) else { continue }
    ...
    func createText(frame: CGRect) -> CALayer {
        ...
        let text = NSAttributedString(string: "Copyright © 2020 joe momma", attributes: attrs)
